...
function simplePostData(config) {
	...
	//生成签名
	var sign = generateSign($.cookie('customId'),nonce,$.cookie("customToken"));
	//数据加密处理
	...
	//head信息
//	var signConfig = {headers : {'Authorization' : $.cookie('customId')+';'+nonce+';'+sign }};
//	var httpConfig = $.extend(true, config.httpConfig, signConfig);
	...
	config.$http({
		...
		url: url,
		data: data,
		headers: {'Authorization' : $.cookie('customId')+';'+nonce+';'+sign }
 ...
 * 生成签名
 */
function generateSign(customId,nouce,customToken){
	return CryptoJS.SHA1(customToken + customId + ":" + nouce)
}

function generateAuthorization(){
	return $.cookie('customId')+';'+nonce+';'+generateSign($.cookie('customId'),nonce,$.cookie("customToken"));
}
...
function getSignUrl(url){
	var sign = generateSign($.cookie('customId'),nonce,$.cookie("customToken"));
	return url+'?authorization='+$.cookie('customId')+';'+nonce+';'+sign;
}
